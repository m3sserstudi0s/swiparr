...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (config.SEERR_URL && config.SEERR_API_KEY) {
    const res = await fetch(`${config.SEERR_URL}/api/v1/request`, {
      ...
      body: JSON.stringify({ mediaType: row.mediaType, mediaId: row.tmdbId }),
    });

    if (!res.ok && res.status !== 409) {
      const text = await res.text();
      console.warn(`[ApproveRequest] Seerr failed for row ${rowId}: ${res.status} ${text}`);
      return NextResponse.json({ error: `Seerr returned ${res.status}` }, { status: 502 });
    }
  } else {
    console.warn(`[ApproveRequest] Seerr not configured — approving row ${rowId} without forwarding`);
  }
